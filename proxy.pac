var wall_proxy = "SOCKS5 127.0.0.1:3389; SOCKS 127.0.0.1:3389;";
...
function FindProxyForURL(url, host) {
  ...
  // WANDA Intranet hosts
  var connection = WandaProxyForURL(url, host);
  if (connection === 'DIRECT') {
    return direct;
  } else if (/^Proxy/.test(connection)) {
    // reset nowall_proxy to WANDA web proxy
    nowall_proxy = connection;
  }

  // Internet hosts, inside GFW
  if (isInDomains(white_domains, host) === true) {
    return nowall_proxy;
  }
  // Internet hosts, outside GFW
  return wall_proxy;
}

// Keep update-to-date with: http://autoproxy.wanda.cn/proxy.pac
function WandaProxyForURL(url, host) {
  if (
    dnsDomainIs(host, "music.wanda.cn")
  ) {
  ...
  else if (isInNet(host, "169.224.11.0", "255.255.255.0")) {
    return "DIRECT";
  }
  else if (isInNet(myIpAddress(), "10.15.194.0", "255.255.254.0")) {
    return "PROXY 10.0.4.6:8080";
  }
  else if (isInNet(myIpAddress(), "10.15.114.0", "255.255.255.0")) {
    return "PROXY 10.0.4.6:8080";
  }
